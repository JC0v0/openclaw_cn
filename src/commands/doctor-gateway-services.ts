...
export async function maybeRepairGatewayServiceConfig(
  ...
  if (needsNodeRuntime && !systemNodePath) {
    ...
    note(
      "未找到系统 Node 22+。通过 Homebrew/apt/choco 安装并重新运行诊断以从 Bun/版本管理器迁移。",
      "网关运行时",
    );
  }
  ...
  const repair = needsAggressive
    ? await prompter.confirmAggressive({
        message: "现在用当前默认值覆盖网关服务配置？",
        initialValue: Boolean(prompter.shouldForce),
      })
    : await prompter.confirmRepair({
        message: "现在将网关服务配置更新为推荐的默认值？",
        initialValue: true,
      });
  if (!repair) {
    return;
...
export async function maybeScanExtraGatewayServices(
  ...
  if (legacyServices.length > 0) {
    const shouldRemove = await prompter.confirmSkipInNonInteractive({
      message: "现在移除旧版网关服务（clawdbot/moltbot）？",
      initialValue: true,
    });
    if (shouldRemove) {
  ...
  note(
    [
      "建议：大多数设置每台机器运行一个网关。",
      "一个网关支持多个代理。",
      "如果需要多个网关（例如，在同一主机上运行救援机器人），请隔离端口 + 配置/状态（参见文档：/gateway#multiple-gateways-same-host）。",
    ].join("\n"),
    "网关建议",
  );
}
