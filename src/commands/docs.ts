...
function resolveNodeRunner(): NodeRunner {
  ...
  throw new Error("缺少 pnpm 或 npx；请安装一个 Node 包运行器。");
}
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    ...
    if (isRich()) {
      ...
      runtime.log(`${theme.muted("搜索:")} ${formatCliCommand('openclaw docs "your query"')}`);
    } else {
      runtime.log("文档: https://docs.openclaw.ai/");
      runtime.log(`搜索: ${formatCliCommand('openclaw docs "your query"')}`);
    }
    return;
  }
