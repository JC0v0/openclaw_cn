...
export async function channelsAddCommand(
  ...
  if (useWizard) {
    ...
    if (selection.length === 0) {
      await prompter.outro("未选择任何渠道。");
      return;
    }

    const wantsNames = await prompter.confirm({
      message: "是否为这些账户添加显示名称？（可选）",
      initialValue: false,
    });
    if (wantsNames) {
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `插件 ${catalogEntry.meta.label} 在安装后无法加载。`
      : `未知渠道：${String(opts.channel ?? "")}`;
    runtime.error(hint);
    runtime.exit(1);
    return;
  }
  ...
  if (!plugin?.setup?.applyAccountConfig) {
    runtime.error(`渠道 ${channel} 不支持添加。`);
    runtime.exit(1);
    return;
  }
  const accountId =
  ...
  await writeConfigFile(nextConfig);
  runtime.log(`已添加 ${channelLabel(channel)} 账户 "${accountId}"。`);
}
