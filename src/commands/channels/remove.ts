...
export async function channelsRemoveCommand(
  ...
  if (useWizard && prompter) {
    ...
    const wantsDisable = await prompter.confirm({
      message: `是否禁用 ${channelLabel(selectedChannel)} 账户 "${accountId}"？（保留配置）`,
      initialValue: true,
    });
    if (!wantsDisable) {
    ...
    if (!channel) {
      runtime.error("需要指定渠道。使用 --channel <名称>。");
      runtime.exit(1);
      return;
    }
    if (!deleteConfig) {
      ...
      const ok = await confirm.confirm({
        message: `是否禁用 ${channelLabel(channel)} 账户 "${accountId}"？（保留配置）`,
        initialValue: true,
      });
      if (!ok) {
        return;
  ...
  if (deleteConfig) {
    if (!plugin.config.deleteAccount) {
      runtime.error(`渠道 ${channel} 不支持删除。`);
      runtime.exit(1);
      return;
    }
    ...
    if (!plugin.config.setAccountEnabled) {
      runtime.error(`渠道 ${channel} 不支持禁用。`);
      runtime.exit(1);
      return;
    }
