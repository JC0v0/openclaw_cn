...
export function registerModelsCli(program: Command) {
  const models = program
    ...
    .option("--agent <id>", "要检查的代理 ID（覆盖 OPENCLAW_AGENT_DIR/PI_CODING_AGENT_DIR）")
    .addHelpText(
      "after",
      () =>
  ...
  models
    ...
    .option("--probe-max-tokens <n>", "探测最大令牌数（尽力）")
    .option("--agent <id>", "要检查的代理 ID（覆盖 OPENCLAW_AGENT_DIR/PI_CODING_AGENT_DIR）")
  ...
  const fallbacks = models.command("fallbacks").description("管理模型备用列表");
  ...
  const imageFallbacks = models.command("image-fallbacks").description("管理图像模型备用列表");

  imageFallbacks
    .command("list")
    .description("列出图像备用模型")
    .option("--json", "输出 JSON", false)
    .option("--plain", "纯文本输出", false)
    .action(async (opts) => {
      await runModelsCommand(async () => {
  ...
  models
    ...
    .option("--max-age-days <days>", "跳过 N 天前的模型")
    .option("--provider <name>", "按提供商前缀筛选")
    .option("--max-candidates <n>", "最大备用候选数", "6")
  ...
  auth
    .command("add")
    .description("交互式认证助手（setup-token 或粘贴令牌）")
    .action(async () => {
      await runModelsCommand(async () => {
  ...
  auth
    .command("setup-token")
    .description("运行提供商 CLI 来创建/同步令牌（需要 TTY）")
  ...
  auth
    ...
    .option("--expires-in <duration>", "可选过期时间（例如 365d、12h）。存储为绝对的 expiresAt。")
    .action(async (opts) => {
      await runModelsCommand(async () => {
  ...
  const order = auth.command("order").description("管理每代理认证配置文件顺序覆盖");

  order
    .command("get")
    .description("显示每代理认证顺序覆盖（来自 auth-profiles.json）")
    .requiredOption("--provider <name>", "提供商 ID（例如 anthropic）")
    .option("--agent <id>", "代理 ID（默认：已配置的默认代理）")
    .option("--json", "输出 JSON", false)
    .action(async (opts, command) => {
      const agent =
        resolveOptionFromCommand<string>(command, "agent") ?? (opts.agent as string | undefined);
  ...
  order
    .command("set")
    .description("设置每代理认证顺序覆盖（将轮换锁定到此列表）")
    .requiredOption("--provider <name>", "提供商 ID（例如 anthropic）")
    .option("--agent <id>", "代理 ID（默认：已配置的默认代理）")
